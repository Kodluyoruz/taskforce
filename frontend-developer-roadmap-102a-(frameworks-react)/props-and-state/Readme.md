...
React props, "properties" kelimesinin kısaltılmasıdır. Aynı state'te olduğu gibi component hakkında data depoladığımız yerdir fakat props yalnızca okunabilen (read-only, hiçbir şekilde mutate edilemezler yani değiştirilemezler) objelerdir. Kullanım amacı component'ler arası data paylaşımı yapmaktır. Bir component diğer component'e props geçerek data aktarır. Props, fonksiyon argümanlarına çok benzer. Argümanlar sayesinde nasıl bir fonksiyona data aktarabiliyorsak, props da bir component'ten diğerine data aktarmak için kullanılır.  
