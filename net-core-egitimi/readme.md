# .Net Core

#### Introduction to .NET Core ####
- [What is .NET Core](what-is-net-core)
- [Advantages and Use Cases of .NET Core](advantages-and-use-cases-of-net-core)
- [.NET Core Version Management and Compatibility](net-core-version-management-and-compatibility)
- [Overview of .NET Core Application Types](overview-of-net-core-application-types)

#### Development Tools and Environments ####
- [Introduction to Visual Studio and VS Code](introduction-to-visual-studio-and-vs-code)

#### Console Applications and MVC Introduction  ####
- [Developing Console Applications with .NET Core](developing-console-applications-with-net-core)
- [MVC Architecture and Basic Concepts](mvc-architecture-and-basic-concepts)
- [Simple MVC Blog App](simple-mvc-blog-app)

#### Rest & HTTP ####
- [Rest principles](rest-principles)
- [HTTP Methods Introduction](http-methods-intro)
- [HTTP Status Code](http-status-codes)
- [HTTP Get](http-get)
- [HTTP post](http-post)
- [HTTP Put](http-put)
- [HTTP Delete](http-delete)
- [HTTP Patch](http-patch)

#### Web API & Best Practices ####
- [API Design](api-design)
- [URL Structure and Naming](url-structure-and-naming)
- [Data Formatting & Serialization](data-formatting-serialization)
- [Versioning](versioning)

#### Web API Development Basics ####
- [ASP.NET Core Web API](asp-net-core-web-api)
- [API Project Structure](api-project-structure)
- [First API Project](first-api-project)
- [API Routing](api-routing)
- [API with HTTP Methods](api-with-http-methods)
- [Model Binding and Validation](model-binding-and-validation)

#### Data Access ####
- [Entity Framework](entity-framework)
- [Code-First Development](code-first-development)
- [Creating Entity Classes](creating-entity-classes)
- [Configuring Relationships](configuring-relationships)
- [Working with DbContext](working-with-dbcontext)
- [Migrations in Code-First](migrations-in-code-first)
- [Seed Data](seed-data)
- [DB-First Development](db-first-development)
- [Basic CRUD](basic-crud)
- [Transactions](transactions)
- [Loading Strategies](loading-strategies)

#### Advanced API ####
- [DI & Service Lifetimes](di-service-lifetimes)
- [Negotiation and Formatters](negotiation-and-formatters)
- [Routing and versioning ](routing-and-versioning)
- [Middlewares](middlewares)
- [Filters](filters)

#### Security ####
- [Identity API](identity-api)
- [JWT](jwt)
- [Authorization](authorization)
- [Authorization Filters](authorization-filters)
#### Caching ####
- [Caching](caching)
  
#### NLayer ####
- [NLayer](nlayer)
